...
untidy_cor <- untidy %>% 
  select(-c(1:6))
cor_conservatism_ocean <- corr.test(untidy_cor)
cor_conservatism_ocean
```

```{r correlations heatmap, include = FALSE}
...
ggplot(tidy, aes(x = personality_score, y = conservatism_score, color = personality_trait)) +
  ...
  labs(title = "Social Conservatism by Personality Traits", subtitle = "Big 5 traits self-reported by adults aged 20-68",
       x= "How strongly participants identify with the personality trait", y = "Social Conservatism", 
...
To better understand the relation between social and economic conservatism, we examined the correlation between social and economic conservatism scores, and found it to be r = 0.65. 
